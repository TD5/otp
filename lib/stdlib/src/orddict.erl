...
-export([fold/3,map/2,filter/2,merge/2,merge/3]).
...
map(F, []=Nil) when is_function(F, 2) -> Nil.
...
filter(F, []=Nil) when is_function(F, 2) -> Nil.
...
If a key occurs in both dictionaries, the value from `Orddict2` is used.
""".
-spec merge(Orddict1, Orddict2) -> Orddict3 when
      Orddict1 :: orddict(Key, Value),
      Orddict2 :: orddict(Key, Value),
      Orddict3 :: orddict(Key, Value).
...
merge([], D2) -> D2;
merge(D1, []) -> D1;
merge([{K1,_}=E1|D1], [{K2,_}=E2|D2]) when K1 < K2 ->
    [E1|merge(D1, [E2|D2])];
merge([{K1,_}=E1|D1], [{K2,_}=E2|D2]) when K1 > K2 ->
    [E2|merge([E1|D1], D2)];
merge([{K1,_}|D1], [{_K2,V2}|D2]) ->	%K1 == K2
    [{K1,V2}|merge(D1, D2)].
