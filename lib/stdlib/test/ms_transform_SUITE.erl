...
optimise_equality_guards_ets_compilation(Config) when is_list(Config) ->
    ...
    ?assertMatch(
       [{{42,'$2'},[],[{{42,'$2'}}]}],
       compile_and_run(
         <<"Needle = 42, ets:fun2ms(fun ({K, V}) when K =:= Needle -> {K,V} end)">>)),
    ?assertMatch(
       [{{42.0,'$2'},[],[{{42.0,'$2'}}]}],
       compile_and_run(
         <<"Needle = 42.0, ets:fun2ms(fun ({K, V}) when K =:= Needle -> {K,V} end)">>)),
    ?assertMatch(
       [{{6,'$2'},[],[{{6,'$2'}}]}],
    ...
    ?assertEqual(
       compile_and_run(
       <<"ets:fun2ms(fun({0,_}) -> X; "
       "fun({6,_}) -> X; "
       "({X,42}) when is_integer(X) -> X end)">>),
       compile_and_run(
       <<"ets:fun2ms(fun({X,Y}) when is_integer(X), "
         "(X =:= 0) orelse (X =:= 6) orelse (Y =:= 42)) -> X end)">>)
    ),
    ok.
...
optimise_equality_guards_ets_equivalence(Config) when is_list(Config) ->
    ...
    Compare(
       ...
       % TODO Don't generate broken queries when two guards refer to one value (the equality value needs to be inlined everywhere)
       <<"ets:fun2ms(fun(X) when is_integer(X), "
         "X =:= 0, "
         "X > 2 -> X end)">>,
       [ordered_set],
...
compile_and_run(Records,Expr) ->
    ...
    Bin =
      case compile:forms(Forms) of
        {ok,tmp,B} -> B;
        E -> error(lists:flatten(io_lib:format("Compilation of match expression failed: ~tp", [E])))
      end,
